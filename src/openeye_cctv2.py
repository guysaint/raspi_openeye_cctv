...
# Utilities
# =========================
def open_picam2(size=PICAM_SIZE, fmt=PICAM_FORMAT):
    picam2 = Picamera2()
...
def read_frame_bgr(picam2):
    frame = picam2.capture_array()  # 이건 항상 "fmt" 그대로의 채널 순서
    if PICAM_FORMAT.upper() == "RGB888":
        # RGB → BGR (OpenCV는 BGR 기준)
        return cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
    elif PICAM_FORMAT.upper() == "BGR888":
        # 이미 BGR이면 그대로 사용
        return frame
    else:
        # 다른 포맷을 쓴다면 안전하게 RGB라고 가정
        return cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
